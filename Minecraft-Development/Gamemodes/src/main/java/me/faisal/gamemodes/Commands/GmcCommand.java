...
public class GmcCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(command.getName().equalsIgnoreCase("gmc")) {
            ...
            if(player.isOp()) {
                if(args.length > 1) {
                    player.sendMessage("Invalid usage! Use /gmc <player>");
                    player.playSound(player.getLocation(), "entity.villager.no", 1, 2);
                } else {
                    if(args.length == 0) {
                        GamemodeChange(player, "creative");
                    } else {
                        ...
                        if(target == null) {
                            ...
                            PrefixMessage(player, "<green> Player " + target.getName() + " has been set to creative mode!");
                            GamemodeChange(target, "creative");
                        }
                    }
                }
            } else {
